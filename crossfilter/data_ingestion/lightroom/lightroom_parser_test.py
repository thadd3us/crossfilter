...
from crossfilter.data_ingestion.lightroom_parser import (
...
def test_parse_lightroom_catalog_real_data(test_catalogs_dir: Path) -> None:
    ...
    test_catalogs = [
        ...
        test_catalogs_dir / "test_catalog_01" / "test_catalog_gps_captions_collections_keywords.lrcat",
        test_catalogs_dir / "test_catalog_02" / "test_catalog_two_more_photos_and_edits.lrcat",
        test_catalogs_dir / "test_catalog_03" / "test_catalog_more_face_tags_gps_edit.lrcat",
    ]
    ...
    for catalog_path in test_catalogs:
        ...
        if df[SchemaColumns.GPS_LATITUDE].notna().any():
            ...
            assert valid_lat.all(), f"Invalid latitudes found: {non_null_lats[~valid_lat].tolist()}"
        ...
        if df[SchemaColumns.GPS_LONGITUDE].notna().any():
            ...
            assert valid_lon.all(), f"Invalid longitudes found: {non_null_lons[~valid_lon].tolist()}"
        ...
        if df[SchemaColumns.RATING_0_TO_5].notna().any():
            ...
            assert valid_ratings.all(), f"Invalid ratings found: {non_null_ratings[~valid_ratings].tolist()}"


def test_parse_lightroom_catalog_with_metadata(test_catalogs_dir: Path) -> None:
    """Test parsing catalog with metadata enabled."""
    catalog_path = test_catalogs_dir / "test_catalog_01" / "test_catalog_gps_captions_collections_keywords.lrcat"
    ...
    metadata_columns = [
        'focal_length', 'aperture', 'flash_fired',
        'import_hash', 'file_format', 'base_name', 'extension',
        'color_labels', 'pick', 'touch_time', 'original_capture_time'
    ]
...
def test_parse_lightroom_catalog_with_keywords(test_catalogs_dir: Path) -> None:
    ...
    if keywords_present.any():
        for keywords in df.loc[keywords_present, 'keywords']:
            assert isinstance(keywords, str), f"Keywords should be a string (for SQLite), got {type(keywords)}"
...
def test_parse_lightroom_catalog_with_collections(test_catalogs_dir: Path) -> None:
    ...
    if collections_present.any():
        for collections in df.loc[collections_present, 'collections']:
            assert isinstance(collections, str), f"Collections should be a string (for SQLite), got {type(collections)}"
...
def test_parse_lightroom_catalog_ignore_collections(test_catalogs_dir: Path) -> None:
    ...
    if 'collections' in df.columns and df['collections'].notna().any():
        ...
        for ignored in ignore_set:
            assert ignored.lower() not in all_collections, f"Ignored collection '{ignored}' found in results"
