...
async def browser() -> Generator[Browser, None, None]:
    ...
    async with async_playwright() as p:
        try:
            ...
            pytest.skip(f"Browser could not be launched (missing system dependencies): {e}")
        try:
            yield browser
        finally:
            await browser.close()
...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_with_sample_data(page: Page, test_server: TestServer, sample_data_loaded: bool) -> None:
