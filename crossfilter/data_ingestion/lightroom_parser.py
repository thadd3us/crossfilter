...
def extract_catalog_from_zip(zip_path: Path) -> Path:
    ...
    try:
        ...
        if len(lrcat_files) > 1:
            logger.warning(f"Multiple .lrcat files found in {zip_path}, using the first one")
...
def build_file_path(root_path: Optional[str], path_from_root: Optional[str], filename: Optional[str]) -> Optional[str]:
...
def parse_lightroom_catalog(catalog_path: Path, config: LightroomParserConfig = None) -> pd.DataFrame:
    ...
    try:
        ...
        # GPS coordinates
        result_df[SchemaColumns.GPS_LATITUDE] = pd.to_numeric(df['gpsLatitude'], errors='coerce')
        result_df[SchemaColumns.GPS_LONGITUDE] = pd.to_numeric(df['gpsLongitude'], errors='coerce')
        
        # Rating (0-5)
        result_df[SchemaColumns.RATING_0_TO_5] = pd.to_numeric(df['rating'], errors='coerce').fillna(0).astype('Int64')
        ...
        if config.include_metadata:
            ...
            result_df['focal_length'] = pd.to_numeric(df['focalLength'], errors='coerce')
        ...
        if config.include_keywords:
            keywords_df = _get_keywords_for_images(catalog_path, result_df[SchemaColumns.UUID_STRING].tolist())
            result_df = result_df.merge(keywords_df, on=SchemaColumns.UUID_STRING, how='left')
        ...
        if config.include_collections:
            collections_df = _get_collections_for_images(catalog_path, result_df[SchemaColumns.UUID_STRING].tolist(), config.ignore_collections)
            result_df = result_df.merge(collections_df, on=SchemaColumns.UUID_STRING, how='left')
        ...
        if extracted_temp_dir:
            try:
                ...
                logger.warning(f"Failed to clean up temporary directory {extracted_temp_dir}: {e}")
...
def _get_keywords_for_images(catalog_path: Path, image_ids: List[str]) -> pd.DataFrame:
    ...
    try:
        ...
        if not keywords_df.empty:
            ...
            grouped = keywords_df.groupby('id_global')['keyword_name'].apply(
                lambda x: sorted(x.dropna().tolist())
            ).reset_index()
            grouped.columns = [SchemaColumns.UUID_STRING, 'keywords']
            grouped[SchemaColumns.UUID_STRING] = grouped[SchemaColumns.UUID_STRING].astype(str)
            return grouped
    except Exception as e:
...
def _get_collections_for_images(catalog_path: Path, image_ids: List[str], ignore_collections: Set[str]) -> pd.DataFrame:
    ...
    ignore_clause = f"AND LOWER(AgLibraryCollection.name) NOT IN ({ignore_str})" if ignore_collections else ""
    ...
    try:
        ...
        if not collections_df.empty:
            ...
            grouped = collections_df.groupby('id_global')['collection_name'].apply(
                lambda x: sorted(x.dropna().tolist())
            ).reset_index()
...
def load_lightroom_catalog_to_df(catalog_path: Path, config: LightroomParserConfig = None) -> pd.DataFrame:
