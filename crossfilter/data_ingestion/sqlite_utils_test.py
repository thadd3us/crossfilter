...
def test_create_or_update_table_new_table(tmp_path: Path) -> None:
    ...
    result = create_or_update_table(engine, "test_table", df)
    
    # Should return True (unique constraint created)
...
def test_create_or_update_table_add_columns(tmp_path: Path) -> None:
    ...
    result = create_or_update_table(engine, "test_table", extended_df)
    
    # Should return True (unique constraint exists)
    assert result is True
...
def test_upsert_with_clean_data_works_properly(tmp_path: Path) -> None:
    ...
    df2 = pd.DataFrame({
        SchemaColumns.UUID_STRING: ["uuid-1", "uuid-3"],  # Update uuid-1, add uuid-3
        SchemaColumns.DATA_TYPE: [DataType.GPX_TRACKPOINT, DataType.GPX_WAYPOINT],
        SchemaColumns.GPS_LATITUDE: [37.7799, 37.7800],  # Updated latitude for uuid-1
