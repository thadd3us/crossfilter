...
class DfIdsFilterRequest(BaseModel):
    ...
    df_ids: list[int] = Field(
        ...,
        description="df_ids from the plot (could be from lasso selection, visible area, etc.)",
    )
    event_source: ProjectionType = Field(
        ..., description="Which plot type this filtering comes from"
    )
    filter_operator: FilterOperatorType = Field(
        ..., description="Type of filter operation to apply"
    )
