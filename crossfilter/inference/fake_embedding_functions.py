...
def compute_image_embeddings(
    ...
    Note: This function creates a new embedder instance each time.
    Consider using FakeEmbedder class directly for consistency.
    """
...
def generate_captions_from_image_embeddings(
    image_embeddings: list[np.ndarray], max_length: int = 50
) -> list[str]:
    """
    Backward compatibility function for generate_captions_from_image_embeddings.
    
    Note: This function creates a new embedder instance each time.
    Consider using FakeEmbedder class directly for consistency.
    """
    embedder = FakeEmbedder()
    return embedder.generate_captions_from_image_embeddings(image_embeddings, max_length)