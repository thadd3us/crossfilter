...
def create_clip_embedding_plot(
    ...
    temp_geo_state = GeoProjectionState(clip_embedding_projection_state.projection_state.max_rows)
    ...
    # Update the plot to reflect that this is CLIP embedding space, not geographic space
    fig.update_layout(
        title=title or "CLIP Embedding Semantic Similarity Space",
        map=dict(
            style="carto-positron",  # Use a cleaner style for abstract space
        ),
    )
