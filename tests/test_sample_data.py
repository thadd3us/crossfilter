...
def test_sample_100_jsonl_loads():
    ...
    required_columns = [
        "UUID_LONG", "GPS_LATITUDE", "GPS_LONGITUDE", "TIMESTAMP_UTC"
    ]
    for col in required_columns:
        assert col in df.columns, f"Required column {col} missing"
        assert not df[col].isna().all(), f"Column {col} is all null"
    
    # Verify UUID_LONG values are ascending integers from 0 to 99
    assert df["UUID_LONG"].tolist() == list(range(100)), "UUID_LONG should be 0-99"
    
    # Verify GPS coordinates are within valid ranges
    assert df["GPS_LATITUDE"].between(-90, 90).all(), "GPS_LATITUDE out of range"
    assert df["GPS_LONGITUDE"].between(-180, 180).all(), "GPS_LONGITUDE out of range"
    
    # Verify TIMESTAMP_UTC is properly parsed as timezone-aware datetime
    assert df["TIMESTAMP_UTC"].dtype.name == "datetime64[ns, UTC]", "TIMESTAMP_UTC should be UTC timezone-aware"
...
def test_sample_data_has_temporal_diversity():
    ...
    time_range = df["TIMESTAMP_UTC"].max() - df["TIMESTAMP_UTC"].min()
