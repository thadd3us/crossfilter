...
from crossfilter.data_ingestion.gpx_parser import generate_uuid_from_components, load_gpx_file_to_df
...
def test_generate_uuid_from_components() -> None:
    ...
    uuid1 = generate_uuid_from_components(
        timestamp1, 37.7749, -122.4194,
        timestamp2, 37.7750, -122.4195,
...
def test_load_gpx_file_to_df_valid_file(tmp_path: Path) -> None:
...
<gpx version="1.1" creator="test">
    ...
    # Should have 3 points total (2 trackpoints + 1 waypoint)
    assert len(df) == 3
    
    # Check data types
    trackpoints = df[df[SchemaColumns.DATA_TYPE] == DataType.GPX_TRACKPOINT]
    waypoints = df[df[SchemaColumns.DATA_TYPE] == DataType.GPX_WAYPOINT]
    ...
    # Check UUID strings are valid
    for uuid_str in df[SchemaColumns.UUID_STRING]:
        assert uuid.UUID(uuid_str)  # Should be valid UUID
    
    # Check timestamps are properly converted to UTC
    assert df[SchemaColumns.TIMESTAMP_UTC].dtype.name == "datetime64[ns, UTC]"
    
    # Check coordinates are within valid ranges
    assert all(df[SchemaColumns.GPS_LATITUDE].between(-90, 90))
    assert all(df[SchemaColumns.GPS_LONGITUDE].between(-180, 180))
    
    # Check source file is set correctly
    assert all(df[SchemaColumns.SOURCE_FILE] == str(gpx_file))
    
    # Check file size is set
    assert all(df[SchemaColumns.SIZE_IN_BYTES] == gpx_file.stat().st_size)
    
    # Check specific values
    first_trackpoint = trackpoints.iloc[0]
    assert first_trackpoint[SchemaColumns.NAME] == "Test Track"
    assert first_trackpoint[SchemaColumns.GPS_LATITUDE] == 37.7749
    assert first_trackpoint[SchemaColumns.GPS_LONGITUDE] == -122.4194
    
    first_waypoint = waypoints.iloc[0]
    assert first_waypoint[SchemaColumns.NAME] == "Test Waypoint"
    assert first_waypoint[SchemaColumns.CAPTION] == "A test waypoint"
    assert first_waypoint[SchemaColumns.GPS_LATITUDE] == 37.7751
    assert first_waypoint[SchemaColumns.GPS_LONGITUDE] == -122.4196


def test_load_gpx_file_to_df_empty_file(tmp_path: Path) -> None:
...
def test_load_gpx_file_to_df_trackpoints_without_time(tmp_path: Path) -> None:
...
<gpx version="1.1" creator="test">
    ...
    # Should return empty DataFrame because trackpoints without timestamps are ignored
    assert len(df) == 0
